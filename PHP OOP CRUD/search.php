...
<body>
    ...
    <?php
    $search = $_POST['search'];
    $ans = $obj->Search($search);
    if (isset($_POST['search_btn'])) {
        if (empty($_POST['search'])) {
            echo "<script>alert('Enter ID or Name !')</script>";
        ...
        <table class="table text-center table-bordered table-hover table-striped">
            ...
            ?>
                <tr>
                    ...
                    <td><a href="delete.php?id=<?php echo $data['id'] ?>"><button onclick="return Del()" class="btn-lg btn-danger p-2">Delete</button></a></td>
                </tr>
        <?php
            }
        } else {
            echo "<script>alert('Invalid ID or  Name')</script>";
            echo "<script>window.location='index.php'</script>";
        }
